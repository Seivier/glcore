...
public:
  ...
  SharedBuffer(cl_context context, const std::vector<float> &vertices)
    ...
    if (!mem || err != CL_SUCCESS) {
      std::cerr << "Error: failed to create buffer from " << vbo_ << " (" << err
                << ")\n";
    }
  };
  void build() override {
...
private:
  void setup_compute() {
    ...
    err = clGetContextInfo(cl.context, CL_CONTEXT_DEVICES, sizeof(device_ids),
                           device_ids, &returned_size);
    if (err) {
      std::cerr << "Error: Failed to retrieve compute devices for context!\n";
      return;
    }
    ...
    if (!device_found) {
      std::cerr << "Error: Failed to locate compute device!\n";
      return;
    }
